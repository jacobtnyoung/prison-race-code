...
load( "prisoner_racial_code_corclass_REVISION.R" )
...
# ================================================================== #
# Create the correlation matrices for each schematic class.

# +++++++++++++++++++++++++++ #
# Recreate the correlation matrices for plotting here.
cor.list <- NULL
n.modules <- length( c.code.cor$modules )
for ( i in 1:n.modules ){
  cor.list[[i]] <- c.code.cor$modules[[i]]$cormat
...
# create the list of significance levels for the correlations.
p.mats <- cor.list
for ( i in 1:n.modules ){
  p.mats[[i]] <- cor_pmat( cor.list[[i]] )
}
...
grid.arrange( 
  cor.plots[[1]], cor.plots[[2]], cor.plots[[3]], cor.plots[[4]],
  nrow = 2, ncol = 2
)
...
# Create a function to rescale the correlation matrix by the max.
# corr.mat.rescale <- function( cormat ){
#   diag( cormat ) <- 0
#   rescaled.cormat <- cormat / max( abs( cormat ) )
#   return( rescaled.cormat )
# }

# # Filter the correlations based on the significance level.
# for ( i in 1:n.modules ){
#  cor.list[[i]] <- cor.filter( cor.list[[i]], pcutoff = 0.50 )
# }

# # Filter the correlations based on the size of the correlation.
# cor.size.cut = 0.2 # set the size at which to cut the ties.
# for ( k in 1:n.modules ) {
#   for ( i in 1: dim( cor.list[[k]] )[1] ){
#     for ( j in 1: dim( cor.list[[k]] )[2] ){
#       if ( abs( cor.list[[k]][i,j] ) < cor.size.cut ) {
#         cor.list[[k]][i,j] <- 0
#       }
#     }
#   }
...
# Create a function to shade the edges.
# edge.col.create <- function( net.edges ){
#   col.set <- colorRampPalette( brewer.pal( 9, "Greys" ) )( length( net.edges ) )
#   col.adjust <- abs( net.edges )*100   # multiple by 100 here to create an integer.
#   col.adjust[col.adjust < 1] <- 1 # recode those less than 1 to be 1 so they are not dropped.
#   edge.cols <- col.set[col.adjust]
#   return( edge.cols )
# }

# Create a function to filter the insignificant correlations.
# cor.filter <- function( cormat, N.vars = dim( cormat )[1], pcutoff = 0.05 ){
#   diag( cormat ) <- 0
#   suppressWarnings(
#     tvalues <- cormat * sqrt( ( N.vars - 2 ) / ( 1 - cormat^2 ) )
#   )
#   cutoff <- abs( qt( pcutoff/2, N.vars ) )
#   cormat[ abs( tvalues ) < cutoff ] <- 0
#   return( cormat )
# }
